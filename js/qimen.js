    ...
    var procSteps = [
      {
        t: '一、记下时间，排出四柱',
        d: [
          '先把起局这一刻的北京时间，换成年、月、日、时四组干支（就是俗称的"八字"记时法）。要是过了晚上 23 点，按老规矩算作第二天。',
          '年柱：' + sz.year + '（一年从立春算起，不是从大年初一）',
          '月柱：' + sz.month + '（换月看节气，不看阴历初一）',
          '日柱：' + sz.day + '（当天的干支）',
          '时柱：' + sz.hour + '（每两小时算一个时辰，本时辰地支为' + sz.hourBranch + '）'
        ]
      },
      {
        t: '二、看节气，找"符头"，分三元',
        d: [
          '查一下这一刻处在哪个节气里：本局在「' + ju.term + '」之内。',
          '再从日柱往前找最近的甲日或己日，作这一小段日子的"领头日"，行话叫符头。本局符头为 ' + ju.fuTou + '。',
          '看符头的地支属于哪一组，把每个节气分成上、中、下三段（三元）：子午卯酉为上元、辰戌丑未为中元、寅申巳亥为下元。本局为' + ju.yuan + '。'
        ]
      },
      {
        t: '三、定阳遁还是阴遁，用第几局',
        d: [
          '一年分两半：冬至到芒种白天越来越长，用"阳遁"；夏至到大雪黑夜越来越长，用"阴遁"。「' + ju.term + '」属' + ju.dun + '遁。',
          '拿"节气＋三元"去查一张固定的对照表，就得出局数：' + ju.term + '·' + ju.yuan + ' → ' + ju.dun + '遁' + juCn + '局。局数决定下一步从哪个宫开始排。'
        ]
      },
      {
        t: '四、排地盘（先打地基）',
        d: [
          '把戊己庚辛壬癸（六仪）和丁丙乙（三奇）共九个天干，从第' + ju.ju + '宫起，按' + (yang ? '阳遁顺着宫位数往前填（1→2→3……）' : '阴遁倒着宫位数往回填（9→8→7……）') + '，依次放进九个宫。这一层叫"地盘"，是整局的地基，排好就不再动。',
          '地盘：' + diPanStr
        ]
      },
      {
        t: '五、找旬首，定出值符、值使（本局两位"值班班长"）',
        d: [
          '干支记时每十个为一组（一旬），组头叫"旬首"。时柱' + sz.hour + '属' + gzText(xunShou) + '旬，这一旬对应的六仪是' + xunYi + '。',
          '在地盘上找到' + xunYi + '，它在' + GONG_NAME[fuGong] + '宫。这个宫原本坐镇的星「' + STAR_HOME[fuGong] + '」便当选"值符"（九星里的值班班长），原本的门「' + DOOR_HOME[jiGong(fuGong)] + '」便当选"值使"（八门里的值班班长）。'
        ]
      },
      {
        t: '六、转天盘（九星跟着值符走）',
        d: [
          '本时辰的天干是' + hourGan + (hourGan === '甲' ? '（甲不直接上盘，用旬首' + xunYi + '代替）' : '') + '，' + targetGan + '在地盘的' + GONG_NAME[jiGong(shiGanGong)] + '宫。',
          '值符星「' + STAR_HOME[fuGong] + '」带着头上的天盘干，从' + GONG_NAME[fuGong] + '宫转到' + GONG_NAME[jiGong(shiGanGong)] + '宫——值符永远追着时干跑；其余八颗星保持原来的相对次序一起转过去，"天盘"就排好了。'
        ]
      },
      {
        t: '七、飞值使，排八门',
        d: [
          '数一数本时辰离旬首隔了' + steps + '位，值使门「' + DOOR_HOME[jiGong(fuGong)] + '」就从原来的' + GONG_NAME[fuGong] + '宫出发，按' + (yang ? '阳遁顺走' : '阴遁倒走') + steps + '步，落到' + GONG_NAME[shiGong] + '宫。',
          '其余七个门按九宫次序跟着排开，八门就位。'
        ]
      },
      {
        t: '八、排八神',
        d: [
          '最后一层是八位神。从值符落下的' + GONG_NAME[jiGong(shiGanGong)] + '宫开始，按' + (yang ? '阳遁顺排' : '阴遁逆排') + '，依次是：值符、螣蛇、太阴、六合、白虎、玄武、九地、九天。排完这层，一局盘就齐了。'
        ]
      },
      {
        t: '九、标出空亡与驿马',
        d: [
          '空亡：每一旬十个干支只轮得到十个地支里的八个，剩下两个"轮空"的地支就叫旬空——落在那里的事往往暂时没着落。',
          '本局时旬空为' + shiKong.join('') + '（' + gzText(xunShou) + '旬轮空的地支），日旬空为' + riKong.join('') + '。',
          '驿马代表走动和变化：按时支' + sz.hourBranch + '的三合规则取，得' + maXing + '（落' + GONG_NAME[ZHI_GONG[maXing]] + '宫）。盘上标"空""马"的宫，就是它们所在。'
        ]
      }
    ];
